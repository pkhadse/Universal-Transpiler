...
class Parser:
    ...
    # program ::= {statement}
    def program(self):
        self.emitter.headerLine("import java.util.Scanner;")
    ...
    def statement(self):
        ...
        if self.checkToken(TokenType.PRINT):
            ...
            if self.checkToken(TokenType.STRING):
                ...
                self.emitter.emitLine("System.out.println(\"" + self.curToken.text + "\\n\");")
                self.nextToken()
        ...
        # "IF" comparison "THEN" block "ENDIF"
            ...
            while not self.checkToken(TokenType.CB):
                self.statement()

            self.match(TokenType.CB)
            self.emitter.emitLine("}")

        elif self.checkToken(TokenType.ELIF):
            self.nextToken()
        ...
        # "LET" ident = expression
            ...
            if self.curToken.text not in self.symbols:
                self.symbols.add(self.curToken.text)
                # self.emitter.headerLine( self.curToken.text )
        ...
        # "INPUT" ident
            ...
            if self.curToken.text not in self.symbols:
                self.symbols.add(self.curToken.text)
                self.emitter.emitLine("Scanner myobj = new Scanner(System.in);")
                self.emitter.emitLine("float " + self.curToken.text + "= myobj.nextFloat();")


                #if self.checkToken(TokenType.FLOAT):
                   # self.emitter.emitLine("char " + self.curToken.text + ";")
